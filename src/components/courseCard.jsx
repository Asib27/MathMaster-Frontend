    ...
    <Link key={course.id} to={`/courses/${course.id}`}>
      <div className='inline-block p-10 basis-1/3 w-96'>
        <div className='rounded overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 ease-in-out'>
          <img className='w-full p-5' src={course.image} alt='Course Image' />
          ...
          <div className='px-6 pt-4 pb-2'>
            <span className='inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2'>{course.estimatedTime + ' hours'}</span>
            ...
            <span className='inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2'>{(course.completed ?? 0) + '% completed'}</span>
          </div>
        </div>
      </div>
    </Link>
